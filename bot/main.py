...
def welcome_text() -> str:
    return (
        f"Здравствуйте, {escape_html(config.DEMO_OWNER_NAME)}! Меня зовут {escape_html(config.DEVELOPER_NAME)}.\n\n"
        f"Мало свежих отзывов в 2ГИС и на Яндекс.Картах — типичная картина: гости уходят довольными, а на Яндексе и 2ГИС тишина.\n\n"
        f"Сейчас вы пройдёте путь клиента в <b>демо-режиме</b> — оценка, отзыв, скрин, скидка. "
        f"Потом покажу, как это выглядит для руководителя.\n\n"
        f"Нажмите <b>«Запустить демо»</b>."
    )
...
async def cb_rate(query: CallbackQuery) -> None:
    ...
    if s["step"] == "demo_rating":
        if n >= 4:
            ...
            kb = InlineKeyboardMarkup(
                inline_keyboard=[
                    [InlineKeyboardButton(text="Отзыв на Яндекс.Картах", url=config.MAP_LINK_YANDEX)],
                    [InlineKeyboardButton(text="Отзыв на 2ГИС", url=config.MAP_LINK_2GIS)],
                    [InlineKeyboardButton(text="Готов отправить скрин", callback_data="ready_screen")],
                ]
            )
            await query.message.edit_text(
                "🧪 <i>Демо-режим</i>\n\n"
                f"Рады, что вам понравилось. Чтобы получить скидку 10% на следующий визит, оставьте отзыв о салоне <b>{escape_html(config.DEMO_SALON_NAME)}</b> на картах и пришлите скрин опубликованного отзыва.\n\n"
...
async def cb_pos_from_neg(query: CallbackQuery) -> None:
    await query.answer()
    set_step(query.from_user.id, "demo_positive")
    kb = InlineKeyboardMarkup(
        inline_keyboard=[
            [InlineKeyboardButton(text="Яндекс.Карты", url=config.MAP_LINK_YANDEX)],
            [InlineKeyboardButton(text="2ГИС", url=config.MAP_LINK_2GIS)],
            [InlineKeyboardButton(text="Готов отправить скрин", callback_data="ready_screen")],
        ]
    )
    await query.message.answer(
        "🧪 <i>Демо-режим — позитив</i>\n\n"
        "Оставьте отзыв по ссылкам и пришлите скрин в чат.",
        parse_mode="HTML",
        reply_markup=kb,
    )
